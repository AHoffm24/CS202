...
int main()
{
	...
	result = m4 * 2;
	matrix answer(result); //copy constructor
...
//All matrixFactory functions will just create a deep copy of
//a into m
...
//This function will just deallocate the matrix that pass points to
void deallocate(size_t rows, int**& pass)
{
	for(int i = 0; i < rows; i++)
	{
		delete [] pass[i]; //deallocate columns
	}
	delete [] pass; //deallocates rows
	pass = NULL;
	return;
}
